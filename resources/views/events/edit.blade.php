    ...
    <div id="event-create-container" class="col-md-6 offset-md-3">
        ...
        <form action="/events/update/{{ $event->id }}" method="POST" id="event-form" enctype="multipart/form-data">
            ...
            <div class="form-group m-3">
                ...
                @endif
                <div class="form-grup">
                    <input type="checkbox" name="items[]" value="Salas para workshops"
                        {{ is_array($event->items) && in_array('Salas para workshops', $event->items) ? 'checked' : '' }}> Salas para workshops
                    <p class="infra">Ambientes específicos para hands-on e treinamentos.</p>
                </div>
                <div class="form-group">
                    <input type="checkbox" name="items[]" value="Área de networking"
                        {{ is_array($event->items) && in_array('Área de networking', $event->items) ? 'checked' : '' }}>
                    Área de networking
                    <p class="infra">Espaço para interação entre participantes, recrutadores e expositores.</p>
                </div>
                <div class="form-grup">
                    <input type="checkbox" name="items[]" value="Expositores e stands"
                        {{ is_array($event->items) && in_array('Expositores e stands', $event->items) ? 'checked' : '' }}> Expositores e stands
                    <p class="infra">Empresas e startups apresentando produtos e serviços.</p>
                </div>
                <div class="form-grup">
                    <input type="checkbox" name="items[]" value="Área de alimentação"
                        {{ is_array($event->items) && in_array('Área de alimentação', $event->items) ? 'checked' : '' }}> Área de alimentação
                    <p class="infra">Coffee break, food trucks ou praça de alimentação.</p>
                </div>
                <div class="form-grup">
                    <input type="checkbox" name="items[]" value="Wi-Fi de alta velocidade"
                        {{ is_array($event->items) && in_array('Wi-Fi de alta velocidade', $event->items) ? 'checked' : '' }}> Wi-Fi de alta velocidade
                    <p class="infra">Essencial para uso dos participantes.</p>
                </div>
                <div class="form-grup">
                    <input type="checkbox" name="items[]" value="Cronograma bem definido"
                        {{ is_array($event->items) && in_array('Cronograma bem definido', $event->items) ? 'checked' : '' }}> Cronograma bem definido
